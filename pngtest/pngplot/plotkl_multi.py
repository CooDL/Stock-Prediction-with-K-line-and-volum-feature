...
#		print filname, idx
	...
	#	print len(dt_yr)
		...
		#==========Notice==============
		if i%1000==0 and i > 0:
			print('%d Sets Finished in %s.cvs'%(i, file))
	print('Complete Plot %s.csv'%file, 'Sleeping for 30 secs to wait Gnuplot')
	time.sleep(30)
	#===========Classify the Pic===========
	...
	for i, dat in enumerate(tradearray[365:len(tradearray)-1]):
		...
		#notice===============
		if i%1000==0 and i > 0:
			print('%d F_Pics Moved'%i)
	print('deleting dat files')
	os.system('rm *.dat')
